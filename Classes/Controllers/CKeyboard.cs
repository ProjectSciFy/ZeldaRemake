        ...
        public void Update()
        {
            ...
            for (int i = 0; i < pressedKeys.Length; i++) {
                ...
                if (keyBinds.ContainsKey(pressedKeys[i]) && !heldKeys.Contains(pressedKeys[i])) {
                    ...
                    if (pressedKeys[i] == Keys.W || pressedKeys[i] == Keys.A || pressedKeys[i] == Keys.S || pressedKeys[i] == Keys.D || pressedKeys[i] == Keys.Up || pressedKeys[i] == Keys.Left || pressedKeys[i] == Keys.Down || pressedKeys[i] == Keys.Right) {
                        ...
                        if (movementKeys.ContainsKey(Keys.S)) movementKeys[Keys.S] = movementKeys[Keys.S] + 1;
                        if (movementKeys.ContainsKey(Keys.D)) movementKeys[Keys.D] = movementKeys[Keys.D] + 1;
                        if (movementKeys.ContainsKey(Keys.Up)) movementKeys[Keys.Up] = movementKeys[Keys.Up] + 1;
                        if (movementKeys.ContainsKey(Keys.Left)) movementKeys[Keys.Left] = movementKeys[Keys.Left] + 1;
                        if (movementKeys.ContainsKey(Keys.Down)) movementKeys[Keys.Down] = movementKeys[Keys.Down] + 1;
                        if (movementKeys.ContainsKey(Keys.Right)) movementKeys[Keys.Right] = movementKeys[Keys.Right] + 1;
                    }
                }
            }
            foreach (Keys releasedKey in releasedKeys)
            {
                ...
                if (movementKeys.ContainsKey(releasedKey)) {
                    ...
                    if (movementKeys.ContainsKey(Keys.S) && movementKeys[Keys.S] > movePriority) movementKeys[Keys.S] = movementKeys[Keys.S] - 1;
                    if (movementKeys.ContainsKey(Keys.D) && movementKeys[Keys.D] > movePriority) movementKeys[Keys.D] = movementKeys[Keys.D] - 1;
                    if (movementKeys.ContainsKey(Keys.Up) && movementKeys[Keys.Up] > movePriority) movementKeys[Keys.Up] = movementKeys[Keys.Up] - 1;
                    if (movementKeys.ContainsKey(Keys.Left) && movementKeys[Keys.Left] > movePriority) movementKeys[Keys.Left] = movementKeys[Keys.Left] - 1;
                    if (movementKeys.ContainsKey(Keys.Down) && movementKeys[Keys.Down] > movePriority) movementKeys[Keys.Down] = movementKeys[Keys.Down] - 1;
                    if (movementKeys.ContainsKey(Keys.Right) && movementKeys[Keys.Right] > movePriority) movementKeys[Keys.Right] = movementKeys[Keys.Right] - 1;
            ...
            if (movementKeys.Count > 0) {
                ...
                else if (movementKeys.ContainsKey(Keys.A) && movementKeys[Keys.A] == 1) currentMoveKey = Keys.A;
                else if (movementKeys.ContainsKey(Keys.S) && movementKeys[Keys.S] == 1) currentMoveKey = Keys.S;
                else if (movementKeys.ContainsKey(Keys.D) && movementKeys[Keys.D] == 1) currentMoveKey = Keys.D;
                else if (movementKeys.ContainsKey(Keys.Up) && movementKeys[Keys.Up] == 1) currentMoveKey = Keys.Up;
                else if (movementKeys.ContainsKey(Keys.Left) && movementKeys[Keys.Left] == 1) currentMoveKey = Keys.Left;
                else if (movementKeys.ContainsKey(Keys.Down) && movementKeys[Keys.Down] == 1) currentMoveKey = Keys.Down;
                else currentMoveKey = Keys.Right;
                if (newMoveKeyCount < lastMoveKeyCount) keyBinds[currentMoveKey].Execute();
            }
            else new IdleLink(linkState).Execute();
        }
    }
}
