        ...
        public void Update()
        {
            ...
            if (mouseState.LeftButton == ButtonState.Pressed && !game.util.keyPressedTempVariable) {
                ...
                if (ValueX > 450 && ValueX < 560 && ValueY > 285 && ValueY < 345){
                    if (game.neighbors[game.util.roomNumber][0] != 0) game.changeRoom(game.neighbors[game.util.roomNumber][0], Collision.Collision.Direction.up);
                }
                if (ValueX > 145 && ValueX < 225 && ValueY > 460 && ValueY < 555) {
                    if (game.neighbors[game.util.roomNumber][1] != 0) game.changeRoom(game.neighbors[game.util.roomNumber][1], Collision.Collision.Direction.left);
                }
                if (ValueX > 790 && ValueX < 860 && ValueY > 460 && ValueY < 555) {
                    if (game.neighbors[game.util.roomNumber][2] != 0) game.changeRoom(game.neighbors[game.util.roomNumber][2], Collision.Collision.Direction.right);
                }
                if (ValueX > 450 && ValueX < 560 && ValueY > 675 && ValueY < 730) {
                    if (game.neighbors[game.util.roomNumber][3] != 0) game.changeRoom(game.neighbors[game.util.roomNumber][3], Collision.Collision.Direction.down);
                }
            }
        }
    }
